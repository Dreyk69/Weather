...
import '../model/weather_hours_model.dart';
import '../model/weather_today_model.dart';
import '../model/weather_week_model.dart';
...
class WeatherService {
  ...
  static Future<Weather> fetchCurrentWeather({query}) async {
    String url =
        'http://api.weatherapi.com/v1/current.json?key=$_apiKey&q=$query&aqi=no&lang=ru';
  ...
  static Future<List<WeatherHoursModel>> fetchHourlyWeather({query}) async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<WeatherHoursModel> data =
  ...
  static Future<List<WeatherWeekModel>> fetchWeekWeather({query}) async {
    ...
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
      final List<WeatherWeekModel> dataWeek =
          (jsonData['forecast']['forecastday'] as List<dynamic>).map((item) {
